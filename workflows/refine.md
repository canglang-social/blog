# Blog refine workflow

This is the provider-neutral canonical workflow for refining a private Blog
draft. Repository-relative paths below resolve from the repository root.

Read `AGENTS.md` and `editor.md` before acting. The input must be a specific
file under `drafts/`; ask which one when it is not supplied. The draft stays in
`drafts/`, and this workflow never publishes.
...
- For `ai-draft` or `mixed`, prioritize the source trace: re-check every
  factual and first-person claim and name anything that cannot be
  corroborated.
