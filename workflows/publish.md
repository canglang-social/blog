...
This is the provider-neutral canonical workflow for moving an approved private
Blog draft into the public collection. Repository-relative paths below resolve
...
Refuse to publish a draft that has not completed the canonical refine workflow
or still mixes languages. Holding a post is a valid outcome.
...
3. Before moving or deleting anything, assemble a paste-ready provenance block
   ...
   This repository contains no vault integration and this workflow must not
   infer a vault path, read another repository, or write outside Blog. Output
   the paste-ready block and stop before any move or deletion. Continue only
   after the owner explicitly confirms that the provenance has been saved
   durably.
...
5. Move the approved file to `src/content/blog/en/YYYY-MM-DD-<slug>.md` for
   English or the matching `src/content/blog/zh/` path for Chinese. Matching
   locale filenames create the language switch. Strip private authorship,
   source material comments, source trace, and other private editorial notes
   from the public file. Use `../../../assets/...` for relative post assets.
